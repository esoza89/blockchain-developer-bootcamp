...
export const loadProvider = (dispatch) => {
	const connection = new ethers.providers.Web3Provider(window.ethereum) // window.ethereum is the connection with metamask
    dispatch({ type: 'PROVIDER_LOADED', connection}) //passed the action and the connection
...
export const transferTokens = async (provider, exchange, transferType, token, amount, dispatch) => {
    ...
    try {
        const signer = await provider.getSigner()//sign with metamask
...
export const makeBuyOrder = async (provider, exchange, tokens, order, dispatch) => {
    ...
    //send amount with respect to first token
